...
# ========================================
# RENDER NAVIGATION IMMEDIATELY (no Snowflake needed)
# ========================================
...
# ========================================
# SNOWFLAKE CONNECTION (lazy — only when needed)
# ========================================
@st.cache_resource
...
def run_query(sql):
    ...
    except (snowflake.connector.errors.DatabaseError, snowflake.connector.errors.OperationalError):
...
# ========================================
# DATA LOADING — wrapped so page renders even if Snowflake is waking up
# ========================================
def load_page_data():
    """Connect to Snowflake and load sidebar filters. Shows spinner while loading."""
    # Sidebar filters
    risk_cats = run_query("""
    ...
    if len(plans) > 0:
        selected_plans = st.sidebar.multiselect(
            "Plan Type",
            options=plans['PLAN_TYPE'].tolist(),
            default=plans['PLAN_TYPE'].tolist()
        )
    else:
        selected_plans = []
...
# Connect + load filters with a visible status message
try:
    with st.spinner("Connecting to Snowflake and loading data..."):
        where_clause = load_page_data()
    data_loaded = True
except Exception as e:
    st.error(f"Could not load data: {e}. Please refresh the page.")
    data_loaded = False
...
if active_tab == "Dashboard":
    ...
    if not data_loaded:
        st.info("Waiting for Snowflake connection. Please refresh in a moment.")
    else:
        kpis = run_query(f"""
            SELECT
